...
def build_square_geometry(N, radius):
    """
    Construit un maillage de N points sur le cercle de rayon 'radius'.
    Retourne:
      coords : tableau (N,2) contenant les coordonnées (x_j,y_j) du bord
...
def build_matrices(coords, normals, ds):
    """
    Construit les matrices G et H pour la formulation BEM (collocation élément constant).
    
    G_{jk} ~ G_2D(x_j, x_k) * ds
    H_{jk} ~ dGdn_2D(x_j, x_k, n_k) * ds
    
    On met 0.0 sur la diagonale de H (car le terme diagonal en double-couche
    est géré séparément comme 0.5 * phi_j).
    """
    N = coords.shape[0]
    G = np.zeros((N, N))
    H = np.zeros((N, N))
    ...
    for j in range(N):
        ...
        for k in range(N):
            ...
            n_k = normals[k,:]
            if j != k:
                G[j,k] = G_2D(x_j, x_k) * ds
                H[j,k] = dGdn_2D(x_j, x_k, n_k) * ds
            else:
                # Sur la diagonale, on met 0 (et on gérera +0.5 phi_j dans le système)
                G[j,k] = 0.0
                H[j,k] = 0.0
    return G, H
...
if __name__ == "__main__":
    ...
    pcm1 = axes[0].pcolormesh(X, Y, phi_num, shading='auto', cmap='jet', vmin=-1, vmax=1)
    #pcm1 = axes[0].pcolormesh(X, Y, phi_num, shading='auto', cmap='jet')
    axes[0].set_title("Solution numérique (BEM)")
    axes[0].set_aspect('equal')
    plt.colorbar(pcm1, ax=axes[0])
    
    # (b) Solution exacte
    pcm2 = axes[1].pcolormesh(X, Y, phi_exact, shading='auto', cmap='jet')
    axes[1].set_title("Solution exacte (x)")
    axes[1].set_aspect('equal')
    plt.colorbar(pcm2, ax=axes[1])
    
    # (c) Carte d'erreur
    pcm3 = axes[2].pcolormesh(X, Y, error_map*100, shading='auto', cmap='jet',vmin=0, vmax=10)
    axes[2].set_title("Erreur (num - exact)")
    ...
    plt.savefig(f"laplace_bem_solution_n{N}.png")